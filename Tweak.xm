...
void printBreakApart(NSString *str) {
    ...
    while (i < length) {
        if (i + width > length)
            width = length - i;
        HBLogInfo(@"%@", [str substringWithRange:NSMakeRange(i, width)]);
        i += width + 1;
    }
}

%hookf(CFCharacterSetRef, CreateCharacterSetForFont, CFStringRef const name) {
    HBLogInfo(@"CreateCharacterSetForFont(%@)", name);
...
%ctor {
    ...
    if (IS_IOS_OR_NEWER(iOS_10_0)) {
        ...
        HBLogInfo(@".AppleColorEmojiUI Character Set (libGSFontCache):");
